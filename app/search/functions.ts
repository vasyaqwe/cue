...
export const list = createServerFn({ method: "GET" })
   ...
   .validator(
      zodValidator(z.object({ organizationId: z.string(), query: z.string() })),
   )
      ...
      const [matchingIssues, issuesWithMatchingComments] = await Promise.all([
         await context.db
            .select({
               ...
               isComment: sql<boolean>`0`.as("isComment"), // to distinguish from comments
               commentId: sql<string | null>`NULL`.as("commentId"),
            })
            .from(issue)
            .where(
               and(
         ...
         await context.db
            .select({
               ...
               status: issue.status,
               createdAt: issue.createdAt,
               isComment: sql<boolean>`1`.as("isComment"),
