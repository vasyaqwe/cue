...
export interface CacheOptions<T = unknown> {
   ...
   shouldInvalidateCache?: (...args: unknown[]) => boolean | Promise<boolean>
   shouldBypassCache?: (...args: unknown[]) => boolean | Promise<boolean>
   group?: string
   integrity?: unknown
   /**
...
export function defineCachedFunction<T, ArgsT extends unknown[] = unknown[]>(
   ...
   async function get(
      ...
      const _resolve = async () => {
         ...
         if (!isPending) {
            ...
            entry.integrity = integrity as string
            delete pending[key]
            if (validate(entry) !== false) {
               const _promise = useStorage()
   ...
   return async (...args) => {
      ...
      if (opts.transform) {
         value = ((await opts.transform(entry, ...args)) as T) || value
      }
      return value as T
   }
}
